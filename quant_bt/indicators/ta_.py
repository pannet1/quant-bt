...
def True_Range(high: pd.Series, low: pd.Series, close: pd.Series) -> pd.Series:
    ...
    selected_columns = [pl.col("high_low"), pl.col("high_close"), pl.col("low_close")]
    max_values = df.select(selected_columns).max()
    max_column_name = None
    max_value = None

    for column_name, value in zip(max_values.columns, max_values.to_pandas().iloc[0]):
        if max_value is None or value > max_value:
            max_column_name = column_name
            max_value = value

    return df.select(max_column_name).to_series()
