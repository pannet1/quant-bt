...
def True_Range(high:pd.Series, low:pd.Series, close:pd.Series) -> pd.Series:
    ...
    df = pl.DataFrame()
    df = df.with_columns(high_low = high - low, high_close = high - close.shift(1).abs(), low_close = low - close.shift(1).abs())

    df = df.with_columns(pl.max([pl.col("high_low"), pl.col("high_close"), pl.col("low_close")]).alias("tr"))
